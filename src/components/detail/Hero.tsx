...
const DetailHeroSegment: FC<DetailHeroSegmentProps> = ({ detail, detailType, originCountry, Credit }) => {
    ...
    return (
        <div ref={heroRef} className="relative flex flex-col lg:h-[700px] overflow-hidden">
            <div className="relative w-full h-full lg:hidden">
                ...
                />
                <div className="flex gap-8 lg:items-center absolute bg-gradient-to-b from-transparent via-transparent to-background w-full lg:h-full inset-0 p-4 lg:p-16 !z-20"></div>
            </div>
            <BlurImage
                src={backdropPath}
                ...
                alt={detail?.title ?? detail?.original_title ?? detail?.name ?? "Detail Image"}
            />
            <div className="flex gap-8 lg:items-center relative lg:absolute lg:bg-gradient-to-t lg:from-background lg:via-background/70 lg:to-transparent/50 w-full lg:h-[60vh] inset-0 p-4 lg:p-8 !z-20">
                <BlurImage
                    src={posterPath}
                ...
                <div className="flex flex-col gap-8">
                            ...
                            <div className="flex items-center gap-2 lg:order-last">
                                {detail?.genres?.map((item: any) => (
                                    <Link
                                        ...
                                        className="font-bold text-secondary-foreground/50 hover:text-secondary-foreground/90"
                                        key={item.id}
                                    >
                                        {item.name}
                                    </Link>
                    ...
                    <div className="flex flex-col gap-4 lg:gap-12 lg:flex-row">
                        {Credit?.crew?.slice(0, 3).map((item: any) => (
                            <div className="flex flex-col gap-2" key={item.credit_id}>
                                <CardDescription>{item.job}</CardDescription>
                                <Link
                                    to={`/person/${item.id}-${(item.name as string).toLowerCase().replace(/ /g, "-")}`}>
                                    <CardTitle className="text-lg hover:text-muted-foreground">{item.name}</CardTitle>
                                </Link>
                            </div>
                        ))}
                    </div>
